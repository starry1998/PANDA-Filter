...
class ShapeNetDataset(data.Dataset):
    def __init__(self,
        ...
        self.msms_file=self.msms_file[self.msms_file["label_rate"]>=self.lr_rate]
        self.msms_file=self.msms_file[self.msms_file["sn_rate"]>=self.sn_rate]
        # self.msms_file=self.msms_file[self.msms_file["sn_rate"]<0.05]
    ...
    def __getitem__(self, index):
        ...
        if(self.dim==2):
            ...
            return point_noise_set, point_clean_set, mz_label_set, intensity_label_set
        else: 
            point_noise_feature_set=np.concatenate((mz_noise_set, norm_intensity_noise_set, comp_set, sister_set),axis=1)
            point_clean_set=np.concatenate((mz_clean_set,intensity_clean_set),axis=1)
            point_noise_feature_set=point_noise_feature_set.astype(np.float32)
...
if __name__ == '__main__':
    datapath = '/data/lingtianze/zyy/denoise/data/dataset/testdata_set/inde_testset/PASS00215.pkl'
    d = ShapeNetDataset(datapath,dim=13)
    print(len(d))
    # point_noise_set,point_clean_set,mz_label, intensity_label=d[0]
    # print(point_noise_set.shape)
    # dataset_size=len(d)
    # print(dataset_size)
